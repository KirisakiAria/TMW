	...
	Segmenter.prototype.options = {
		...
		animation: {
			...
			translateZ: {
				min: 10,
				max: 65
			}, // We can also use an integer for a specific value.
		...
		if (this.options.parallax) {
				...
				requestAnimationFrame(function() {
					self.pieces.forEach(function(piece) {
						var t = piece.getAttribute('data-parallax-translation'),
							transX = t / (self.dimensions.width) * ev.clientX * 2 - t / 2,
							transY = t / (self.dimensions.height) * ev.clientY * 2 - t / 2;
			...
			this.handleOrientation = function() {
				...
				requestAnimationFrame(function() {
					self.pieces.forEach(function(piece) {
						var t = piece.getAttribute('data-parallax-translation'),
							transX = t / (self.dimensions.width) * y * 2 - t / 2,
							transY = t / (self.dimensions.height) * y * 2 - t / 2;
